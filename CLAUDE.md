...
- **場所**: 名前プリセット（自宅/職場/学校/スーパー/その他）＋自由入力＋「地図で指定」「現在地から」＋通知する範囲（100/300/500m）。「その他」は名前欄をクリアするだけの操作（自由入力を促す）。「地図で指定」「現在地から」を押すと`ShopMapPicker`が**さらに上のポップアップ（`z-[110]`）として**開く（`ShopMapPicker`自体が地図内検索(Nominatim)を内包しているため、外側フォームに住所検索ボックスは不要）
...
- **持ち物**: 自由入力＋よく使う候補チップ（財布/鍵/定期券/パソコン/充電器）のワンタップ追加＋登録済みチップの×で削除
- **プレビュー**: フォーム最下部に常時表示される`previewText(editing)`が生成する文章（例:「平日の終日に職場に着いたとき、財布、鍵を確認してください。」）。入力内容の変更にリアルタイムに追従する。別ポップアップには分離していない（入力と確認を同じスクロール内に置き、常に両方が見える状態を維持する狙い）
