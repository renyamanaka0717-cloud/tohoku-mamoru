...
ほぼすべての機能が `src/app/page.tsx` 1ファイルに集約されている（約3240行）。コンポーネント分割は最小限。
...
| 役割 | 値 | 用途 |
|---|---|---|
| メインアクセント | `#7FAE8C` | **選択中**ファイルタブ・FAB・バッジ・並び替えボタン・週カレンダー選択日 |
...
① 日付表示（`2026年6月13日`）＋ カレンダー・検索・設定アイコン  
② 1週間カレンダー（日〜土）  
③ ファイルタブ（すべて・ユーザー定義タブ・＋）  
④ タイムライン
...
- 選択日: `bg-[#7FAE8C] text-white`、今日（未選択）: `bg-gray-100 text-gray-900`
- 左右スワイプ（dx>50px かつ縦より横が大きい）→ ±7日移動
- 週カレンダー上部の横線なし（`border-t` は削除済み）
...
- 外枠は **`bg-white`**（セージグリーン背景ではない）
- 柔らかい影 `0 2px 6px rgba(0,0,0,0.06)` で浮き上がり感
- **選択中タブ**: `background:'#7FAE8C'`・白テキスト・`marginBottom:'-2px'`・zIndex:10
- **非選択タブ**: `background:'#F3F4F6'`（bg-gray-100 相当）・`color:'#6B7280'`（text-gray-500 相当）
...
- タスク名・メモ・**タグ名**で検索（大文字小文字無視）
- タグ名の部分一致でもヒット
- `Task.tags` はタグ名を直接格納しているため、`globalTags` のルックアップ不要
...
- 検索UIやレイアウトは変更しない
- `SearchPage` のプロップス: `{tasks, onClose, onSelect}`
...
- **メインアクセント**: `#7FAE8C`（セージグリーン）— **選択中**タブ・FAB・選択状態・バッジ
...
- ファイルタブの外枠に `bg-[#7FAE8C]` を使う（現在は `bg-white` + 柔らかい影）
...
3. **大規模リファクタリングを避ける**（約3240行の1ファイル構成は意図的）
...
git rebase --exec "git commit --amend --no-edit --reset-author" origin/<セッションブランチ名>
git push origin <セッションブランチ名> --force-with-lease
```
