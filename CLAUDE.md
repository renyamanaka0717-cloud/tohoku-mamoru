...
**スペイン語追加時に見つかった教訓（UIの固定px幅がラテン文字圏の言語で不足するケース）**: ヘッダーの「空き時間」トグルボタンは`width:language==='ja'?'84px':language==='ko'?'92px':language==='zh-TW'?'84px':'104px'`のように文言の長さに応じて固定pxで幅を出し分けていたが、`headerFreeTimeToggle`のスペイン語訳「Tiempo libre」（12文字）は英語版「Free time」（9文字）より長く、既定の`104px`にesを含めるとテキストがボタンの丸角からはみ出して見えた（Playwrightのスクリーンショットで実際に確認して発覚）。修正: esだけ`118px`の専用値を追加した。**新しい言語を追加する時、英語より単語が長くなりがちな言語（スペイン語・ポルトガル語等）では、英語の`else`分岐に自動的に乗せるのではなく、固定px幅を使っているUI要素がないか探し、実際にPlaywrightでスクリーンショットを撮って確認すること。** ポルトガル語の「Tempo livre」はスペイン語訳と綴りがほぼ同じ長さだったため、`es`と同じ`118px`をそのまま流用した（`language==='pt'?'118px'`）。新しい言語を追加する時、既存言語の訳文と文字数が近ければ既存の専用値をそのまま使い回してよいが、必ずPlaywrightで実際に確認してから流用すること（今回はたまたま同じ長さだっただけで、確認せずに決め打ちしたわけではない）。ベトナム語訳「Thời gian rảnh」（15文字、スペースを含む単語数も多い）はes/ptより明らかに長いため専用の`140px`を追加した（`language==='vi'?'140px'`）。Playwrightのスクリーンショットで実際に収まることを確認済み。タイ語訳「เวลาว่าง」（タイ文字8文字）はラテン文字圏の言語より横幅を取りにくい書字体系のため、既存のjaと同じ程度の余裕で収まると判断し`110px`を追加した（`language==='th'?'110px':'104px'`）。Playwrightのスクリーンショットで実際に余裕を持って収まっていることを確認済み（タイ文字はCJKと違って全角換算はできないため、文字数だけで判断せず必ず実際にレンダリングして確認すること）。インドネシア語訳「Waktu luang」（11文字、スペース含む）はes/ptの「Tiempo libre」「Tempo livre」（各12文字）とほぼ同じ長さのラテン文字なので、確認せず決め打ちはせず一旦既存の`118px`を流用してPlaywrightで検証し、実際に余裕を持って収まることを確認した上でそのまま採用した（`language==='id'?'118px':'104px'`）。

**上記の検証はトグル自身の文字がボタン内に収まるかだけを見ており、ヘッダー行全体（日付タイトル＋トグル＋カレンダー/検索/設定の3アイコン）の合計幅が画面に収まるかは別途検証していなかったため、実機でスペイン語表示時にカレンダー・検索・設定アイコンが画面右端からはみ出て見えない不具合が実際に発生した。** 原因は日付タイトル側（ヘッダー最上段の年月表示、`headerFreeTimeToggle`とは別のロジック）にあった。英語は`` `${MONTH_NAMES_EN[month-1].slice(0,3)} ${year}` ``（例:"Aug 2026"）と月名を3文字に短縮しているのに対し、スペイン語・ポルトガル語だけは`` `${MONTH_NAMES_ES[month-1]} de ${year}` ``のように月名をフルスペルのまま`de`連結子付きで表示していた（例:"septiembre de 2026"）。この長さ＋幅広トグル(118px)＋3アイコンの合計がiPhoneの画面幅を超え、`flex justify-between`で右にはみ出たアイコンがそのまま画面外に消えて操作不能になっていた。修正: es/ptも英語と同じく`.slice(0,3)`で月名を3文字に短縮し、`de`連結子を削除して`"Sep 2026"`/`"Set 2026"`のような英語と同型の短い表記にした（Spanish/Portuguese圏の慣用月略記とも一致する3文字）。**新しい言語のヘッダー表示を確認する時は、トグル単体だけでなく、ヘッダー行のカレンダー・検索・設定アイコンが画面内に収まっているか（`header button`の`boundingBox()`が画面幅を超えていないか）も必ずPlaywrightで確認すること。** vi/th/idの日付タイトルは元から短い/専用の短縮ロジックを持つため今回は対象外だが、同じ問題が起きうる構造（フルスペル月名＋長い連結語）を新しく追加する時は同様に確認すること。

**その後の追加調査で、ベトナム語も同じ理由で実機で「アイコンがはみ出る」ではなく「トグルと日付が近すぎる」不具合が見つかった。** viは`th`/`id`より短いと判断して当初は対象外にしていたが、`headerFreeTimeToggle`の訳「Thời gian rảnh」が9言語中もっとも長く専用幅`140px`（他言語より26px以上広い）を必要としていたため、`` `Tháng ${month}, ${year}` ``（例:"Tháng 9, 2026"）というフルスペル日付タイトルと組み合わさると再び窮屈になっていた。修正は2段階: ① まず根本対策として、日付タイトルの`<span>`に`truncate min-w-0`を、アイコン+トグルのグループに`shrink-0`を付け、親の`flex`コンテナに`gap-3`を追加した。これにより**タイトルがどれだけ長くても、アイコン群は絶対に画面外へ押し出されず、必要ならタイトル側が省略記号で短縮される**（今後どの言語で同種の不具合が起きても、アイコンが消える最悪のケースだけは構造的に起きなくなる安全網）。② その上で、vi単体は`` `T${month} ${year}` ``（例:"T9 2026"、ベトナム語で月を指す一般的な略記）に短縮し、実際に省略記号が出る場面を最小限に抑えた。**新しい言語のヘッダー日付フォーマットを追加する時は、この`truncate`+`shrink-0`の安全網があるからといって手を抜かず、実際に複数のiPhone幅（375/390/428px程度）でPlaywrightのスクリーンショットを撮り、タイトルが省略され過ぎていないか確認すること。**
