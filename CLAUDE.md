...
**追加/編集フォームは項目を並べる形式ではなく、「文章を完成させる」形式（穴埋め文）にしてある。** 理由は「設定画面」ではなく「行動を登録する」体験にするため（情報量・視線移動を減らし、何を設定すればいいか一目でわかるようにする狙い）。
...
- 各`［...］`は`SentenceBlank`コンポーネント（ページ内で定義、`ForgetAlertsPanel`の直前）。タップすると`activeField`（`'location'|'trigger'|'weekday'|'time'|'items'|null`）が切り替わり、文章のすぐ下にその項目のピッカーだけが展開する（アコーディオン方式。他のピッカーが同時に開くことはない）
- **場所**ピッカー: 名前プリセット（自宅/職場/学校/スーパー/その他）＋自由入力＋「地図で指定」「現在地から」＋通知する範囲（100/300/500m）。「その他」は名前欄をクリアするだけの操作（自由入力を促す）。「地図で指定」「現在地から」を押すと`ShopMapPicker`が**さらに上のポップアップ（`z-[110]`）として**開く（`ShopMapPicker`自体が地図内検索(Nominatim)を内包しているため、外側フォームに住所検索ボックスは不要）
- **通知するタイミング**ピッカー: 「到着したら」/「出発したら」の2択（`trigger`）
- **曜日**ピッカー: 「毎日」「平日」「休日」「カスタム」のプリセットボタン＋（カスタム選択時、または既存の組み合わせがどのプリセットにも一致しない場合のみ）7つの丸ボタン。プリセット判定は`dayPreset(weekdays)`が行う
- **時間帯**ピッカー: 開始/終了の`<input type="time">`＋「解除」（任意、指定しなければ終日）
- **持ち物**ピッカー: 自由入力＋よく使う候補チップ（財布/鍵/定期券/パソコン/充電器）のワンタップ追加＋登録済みチップの×で削除

**「次へ」を押すと保存前のプレビュー**（`showPreview`、`z-[105]`のポップアップ）が開き、「{name}を出るとき／{曜日}・{時間帯}・{半径}m／・持ち物1／・持ち物2／を確認してください。」という最終文面を確認してから「登録する」で確定する（`saveEditing()`が実際の保存・位置情報許可チェックを行う）。「戻る」で文章編集に戻れる。

**バリデーション:** 名前・場所・曜日1つ以上・持ち物1つ以上が揃うまで「次へ」ボタンは無効。
