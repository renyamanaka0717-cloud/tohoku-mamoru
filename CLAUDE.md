...
3種類の座標計算が存在する。それぞれの用途を混同しないこと。

```typescript
// 純粋な時刻→Y変換（線形）
const calcDayY = (min:number) => wakeCardTop + WAKE_CARD_H + (min - wakeMin) * PX_PER_MIN;

// piecewise補間（タスク実配置を反映）
const layoutCalcY = (min:number): number => { /* anchors を使った補間 */ };
...
| 用途 | 使用する関数 |
|---|---|
| タスクカード配置 | `groupLayout[i].top`（`calcDayY` + 12px最小間隔） |
| 現在時刻インジケーター | `calcDayY(nowMin)` — 純粋な時刻位置 |
| 空き時間スロット内の時刻ラベル | `calcDayY(m)` |
| ドラッグ青ライン | `layoutYRef`（`layoutCalcY` 経由） |
| タッチY→時刻変換 | `yToTimeRef`（piecewise逆引き） |

**現在時刻インジケーターが `layoutCalcY` を使うとタスク押し下げに引っ張られてズレるため、必ず `calcDayY` を使う。**
...
{date===todayStr()&&nowMin>=wakeMin&&nowMin<=sleepMin&&(
  <div className="absolute flex items-center z-20 gap-1.5"
    style={{top:`${calcDayY(nowMin)-12}px`,left:0,right:0}}>
...
- バッジのみ表示（横線・＋ボタンは削除済み）
- Y座標は **`calcDayY(nowMin)`** を使用（`layoutCalcY` ではない）
...
- 現在時刻インジケーターに `layoutCalcY` を使う（必ず `calcDayY` を使う）
