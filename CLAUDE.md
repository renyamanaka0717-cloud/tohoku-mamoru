...
- **矢印（三角）の向きは`above`（吹き出しが対象の上に出るか下に出るか）に関わらず常に`borderBottom`で統一している**（`position`だけ`above`で`top:-8`/`bottom:-8`を出し分ける）。以前は`above`ごとに`borderTop`/`borderBottom`を逆にして「常に対象を指す」向きにしていたが、`above=true`側と`above=false`側で「どちら向きが正しく見えるか」のフィードバックが噛み合わず、片方を直すともう片方が逆戻りする、という無限ループになった実績がある（2つの吹き出し位置は同じ三角スタイルの分岐を共有しているため、一方だけを見た「逆にして」を毎回両方に適用してしまっていたのが原因）。**向きを変える依頼が来たら、まず`above`の両方の分岐でどちらの見た目になっているかを確認し、依頼のあったステップ側の分岐だけを変更すること**（現在のように両分岐が同じ値になっている場合は、position分岐だけ残して`borderBottom`/`borderTop`を出し分ける形に戻す）
