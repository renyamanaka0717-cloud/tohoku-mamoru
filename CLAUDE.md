...
**「あとでやる」一覧内の並び替え（ドラッグ、`sortDir===null` の既定表示時のみ）:** ピン留めタスクは対象外（常に先頭固定・ドラッグ不可）。長押し500ms＋vibrateで開始する点は他のドラッグ機能と共通だが、これは**タイムラインへのスケジュール化ドラッグとは別物**——BottomTabsの「あとでやる」一覧からタイムラインへ直接ドラッグ&ドロップする機能は廃止済み（`FreeTimeCard`のチップからタイムラインへのドラッグ&ドロップは引き続き別途存在し、そちらは影響を受けていない。プロダクトツアーの最終ステップが対象にしているのもそちらのFreeTimeCardのチップ）。

- ジオメトリ（各行の初期 top/height）は `getBoundingClientRect()` でドラッグ開始時に1回だけ計測し `reorderSlots` に固定する。以後は指の移動量（ドラッグ開始位置からの累積 `dy`）だけで「今どの固定スロットに重なっているか」を判定し、`reorderIds`（並び替え中の一覧内 `id` の順序、ローカル state）を更新する。スロット自体は再計測しない
- 並び替え結果を親の `tasks` state（`onReorderLater` prop）に反映するのは**ドロップ時（`touchend`）に1回だけ**。ドラッグ中に毎回 `setTasks` すると全体の再レンダーが重くなるため
- `reorderLaterTasks(orderedIds)`（App コンポーネント）は、`tasks` 配列内でのそのタスク群の**スロット位置はそのまま**（他の日付・時間指定タスクとの相対位置を変えない）、中身の並び順だけ `orderedIds` に差し替える実装。他のタスク（あとでやる以外）の並びには影響しない

**過去の不具合（実機で「ドラッグしづらい」）:** 初回実装は、長押し待ち（500ms）中も含めて行要素に直接付けたReactの合成`onTouchMove`だけで移動判定していた。①合成`onTouchMove`はどんな微小な移動（指の自然な震え、数px）でも即座に長押しをキャンセルしてしまい長押し自体がほぼ成立しない、②実際のドラッグ中もReactの合成タッチイベントはpassive登録されており`e.preventDefault()`が効かず、ドラッグ中に一覧が裏でスクロールしてしまいドラッグと競合する、の2つの原因で「ドラッグしづらい」不具合になった。修正: 長押し待ち中の移動判定には12px程度の許容範囲を設け（`onReorderPreMove`）、実際のドラッグが始まったら（`reorderId`が立ったら）`useEffect`で`document`に直接`{passive:false}`の`touchmove`リスナーを付けて`preventDefault()`する方式に変更した（App本体のタイムラインドラッグ`startDrag`と同じパターン）。新しく行内タッチドラッグ機能を作る時はこのパターンに倣うこと。
...
英語対応を一通り終えた際に確立した手順。次に3言語目（例: `es`）を追加する時はこの順で進める。

**① Web/JS側（`src/app/components/I18n.tsx`）**

1. `export type Language = 'ja'|'en';` に新しいコードを追加（例: `'ja'|'en'|'es'`）
2. `STRINGS`の全キーに新言語のフィールドを追加。キー数が多いので、既存の`{ja,en}`を`{ja,en,es}`に機械的に広げるスクリプト的アプローチも検討する（ただし翻訳の質は人力レビューが必要）
3. `detectLanguage()`（端末言語からの自動判定フォールバック）を更新。現状は「`ja`で始まればja、それ以外はen」という二択ロジックなので、新言語の判定条件を追加する
4. `DAY_NAMES_EN`/`MONTH_NAMES_EN`のような言語別の固定配列（曜日名・月名）がある箇所は同様に新言語版配列を追加し、対応する`language==='ja'?...:...`の2値分岐をすべて3値以上の分岐に書き換える（`page.tsx`内に散在しているため後述の監査grepで洗い出す）
5. 設定 → 表示設定 → 言語ピッカー画面（`sub==='language'`）の選択肢に追加
