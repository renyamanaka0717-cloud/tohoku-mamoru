          ...
          <div className="bg-white rounded-2xl p-5 shadow-xl w-full max-w-xs">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={()=>{setShowDiscard(false);onClose();}}
                className="flex-1 py-2.5 bg-red-500 text-white rounded-xl text-sm font-semibold">破棄する</button>
            </div>
          </div>
        </div>
      )}
...
function Timeline({date,tasks,later,settings,now,onToggle,onEdit,onEditIconSheet,onSchedule,onAddAtTime,onDragStart,dragTaskId,yToTimeRef,layoutYRef,globalTags,todayHistory,onSubtaskToggle,onDragWake,onDragSleep,onCameraClick}:{
  ...
  // CARD_LEFT=116, p-4*2=32 → inner width = screenWidth - 148
  const calcFreeContentH=(tasks:Task[]):number=>{
    ...
    const innerW=(typeof window!=='undefined'?window.innerWidth:375)-116-32;
    let rows=1,rowW=0;
  ...
  const CARD_GAP     = 16;  // px — between icon right edge and card left

  const AXIS_X    = TIME_LABEL_W + AXIS_GAP + ICON_HALF;  // 72
  const CARD_LEFT = AXIS_X + ICON_HALF + CARD_GAP;         // 116
    ...
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 font-sans">
      ...
      <div className="fixed bottom-16 right-4 z-50">
        <button onClick={()=>openAdd()}
          className="w-14 h-14 bg-gray-900 text-white rounded-full shadow-2xl active:bg-gray-700"
