      ...
      {groupLayout.map(({g,top})=>{
        ...
        return [
          <div key={`cap-${g.startTime}`} className="absolute z-10 pointer-events-none"
            ...
            {(()=>{
              const topH=68,overlap=22,botTop=topH-overlap;
              const botH=Math.max(g.h,56)-botTop;
              const Ic1=getTaskIcon(g.tasks[0]?.icon||defaultIconKey(g.tasks[0]?.name||''));
              const Ic2=getTaskIcon(g.tasks[1]?.icon||defaultIconKey(g.tasks[1]?.name||''));
              return(
                <div className="relative" style={{width:'56px',height:`${Math.max(g.h,56)}px`}}>
                  <div className="absolute" style={{top:`${botTop}px`,left:0,width:'56px',height:`${botH}px`,borderRadius:'28px',background:g.tasks[1]?.color||'#D9A3B2',display:'flex',alignItems:'center',justifyContent:'center'}}>
                    <Ic2 size={24} className="text-white"/>
                  </div>
                  <div className="absolute z-10" style={{top:0,left:0,width:'56px',height:`${topH}px`,borderRadius:'28px',background:g.tasks[0]?.color||'#D9A3B2',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 0 0 3px white'}}>
